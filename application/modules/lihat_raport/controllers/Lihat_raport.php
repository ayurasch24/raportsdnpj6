...
class Lihat_raport extends CI_Controller {
    ...
    public function cetak($id_siswa,$tasm) {
        ...
        $q_nilai_sikap_sp = $this->db->query("SELECT sangat_naik, perlu_bimbingan FROM t_nilai_sikap_sp WHERE tasm = '".$tasm."' AND id_siswa = '".$id_siswa."'")->row_array();
        ...
        if (!empty($q_nilai_sikap_sp['selalu'])) {
            ...
            $sangat_baik1        = $list_kd_sp[$sb_1];
            $sangat_baik2        = $list_kd_sp[$sb_2];
            $perlu_bimbingan= $list_kd_sp[$pb];


            $nilai_sikap_spiritual = 'Sangat_baik : '.$sb_1.', '.$sb_2.'; Mulai meningkat pada sikap : '.$perlu_bimbingan;
        } else {
            $sangat_baik1      = '';
            $sangat_baik2       = '';
            $perlu_bimbingan= '';
