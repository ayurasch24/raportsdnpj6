...
class N_sikap_so extends CI_Controller {
    ...
    public function simpan_nilai() {
        ...
        //     $sangat_baik = empty($p['sangat_baik_'.$i]) ? "" : $p['sangat_baik_'.$i];
        //     $perlu_bimbingan = empty($p['perlu_bimbingan_'.$i]) ? "" : $p['perlu_bimbingan_'.$i];
        //     //echo var_dump($sangat_baik);
        //     //echo var_dump($perlu_bimbingan);
        //     if (!empty($sangat_baik)) {
        //         if (in_array($perlu_bimbingan, $sangat_baik)) {
        //             $error[] = "Error baris ".$i." : Isian \"Perlu Bimbingan\" sudah dipakai di isian \"Sangat Baik\"";
        //         }
        //     } else {
        ...
        if (empty($error)) {
            ...
            //     $sangat_baik = implode(",",$p['sangat_baik_'.$i]);
            //     $perlu_bimbingan = $p['perlu_bimbingan_'.$i];
            //     $is_wali = 'Y';
            //     if ($mode_form == "add") {
            //         $this->db->query("INSERT INTO t_nilai_sikap_so (tasm,id_guru_mapel,id_siswa,is_wali,sangat_baik,perlu_bimbingan) VALUES ('$tasm','$id_guru_mapel','$id_siswa','$is_wali','$sangat_baik','$perlu_bimbingan')");
            //     } else {
            //         $this->db->query("UPDATE t_nilai_sikap_so SET is_wali = '".$is_wali."', sangat_baik = '".$sangat_baik."', perlu_bimbingan = '".$perlu_bimbingan."' WHERE id_guru_mapel = '".$id_guru_mapel."' AND tasm = '".$tasm."' AND id_siswa = '".$id_siswa."'");
            //     }
            ...
            for ($i = 1; $i < $p['jumlah']; $i++) {
                ...
                for($j=1;$j<=$p['jumlah_nilai_sosial'];$j++){
                if($p['sangat_baik_'.$i.'_'.$j]==1){
                    $data_sangat_baik.=$j.",";
                }else{
                    $data_perlu_bimbingan.=$j.",";
                }
            }
                $tasm = $this->d['tasm'];
                $id_guru_mapel = $this->d['id_kelas'];
